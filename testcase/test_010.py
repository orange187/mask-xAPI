# !/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2022/9/6 1:59 下午
# @Author  : chencheng
# @File    : test_010.py
# @Remark  : 取消关注指定用户或实体 - v1/user/follow
...
@allure.feature('取消关注指定用户或实体 - v1/user/follow')
class Test_UpdateRate:
    # 参数化
    # @pytest.mark.parametrize()
    def test_Tax(self):
        self.lab = gl.get_value('lab')
        self.caseid = '%s-010' % (self.lab)
        self.ip = gl.get_value('ApiIp')
        self.token = gl.get_value('Authorization')

        # 测试API地址
        self.url = '%suser/follow?toObjectId=177604511&type=twitter&toObjectValue=' % (self.ip)
        print(self.url)
